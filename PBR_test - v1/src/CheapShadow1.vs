...
uniform float u_ShadowStrength1;
uniform float u_ShadowChangeCoefficienct;
uniform float u_ShadowScale1;
...
void main(){
	...
	vec3 lightPos = normalize(lightDir + u_ShadowScale1*worldNormal)*(heightFromGround/lightDir.y); //光照方向标准向量乘以顶点高度与光照向量垂直分量之比。获得一个高度与顶点高度相同的光照方向向量
	...
	v_ShadowStrength = u_ShadowStrength1*max((1.0 - heightFromGround*u_ShadowChangeCoefficienct),0.0);
}