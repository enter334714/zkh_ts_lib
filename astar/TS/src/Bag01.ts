...
export default class Bag01 {
    ...
    * @return 最大价值
    */
    private solveKS(w: number[], v: number[], index: number, capacity: number) {
        ...
        var res: number = this.solveKS(w, v, index - 1, capacity);
    ...
    public knapSack(w: number[], v: number[], C: number) {
        let size = w.length;
